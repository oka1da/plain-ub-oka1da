...
async def remove_fed(bot: BOT, message: Message):
    ...
    if deleted:
        ...
        await message.reply(text=text, del_in=5)
        await bot.log_text(text=text, type="info")
    else:
        ...
        try:
            if message.chat.admin_privileges and message.chat.admin_privileges.can_restrict_members:
                await message.replied.reply(
                    text=f"!dban {reason}", disable_preview=True, del_in=5, block=False
                )
...
async def _perform_fed_task(
    ...
    if failed:
        ...
        resp_str += f"\n<b>{task_type}ned by</b>: <a href='tg://user?id={message.from_user.id}'>{get_name(message.from_user)}</a>"
